...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (path === "system-settings") {
      ...
      if (method === "GET") {
        const rows = await sql<{ key: string; value: string | null }[]>`select key, value from system_settings where key in ('gemini_enabled','gemini_model','gemini_temperature','gemini_api_key','default_title_prompt','default_description_prompt')`;
      ...
      if (method === "POST" || method === "PATCH") {
        const body = typeof req.body === "string" ? JSON.parse(req.body) as Record<string, unknown> : (req.body as Record<string, unknown> || {});
        const allowed = new Set(['gemini_enabled','gemini_model','gemini_temperature','gemini_api_key','default_title_prompt','default_description_prompt']);
